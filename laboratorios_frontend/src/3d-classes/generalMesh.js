export default class GeneralMesh {
  ...
  checkHoveredElements(intersects) {
    let hoveredElements = intersects.map((x) => x.name);
    //CHECK THE STATUS OF ALL ELEMENTS AND CHANGE THE ONES THAT ARE NOT IN HOVER
    for (let i = 0; i < this.getAllMeshes().length; i++) {
      if (hoveredElements.includes(this.getAllMeshes()[i].getName())) {
  ...
  controlColorOfHovered(intersects) {
    ...
    intersects = intersects.map((x) => this.getMeshByName(x.name));
    intersects = intersects.filter(
      (x) => x != null && x.isUserControlled() && x.isOnHover()
    );
    let intersectNames = intersects.map((x) => x.getName());
    for (let i = 0; i < this.getMeshesIfUserControll().length; i++) {
      if (!this.getMeshesIfUserControll()[i].isSelected()) {
        if (
          intersectNames.includes(this.getMeshesIfUserControll()[i].getName())
        ) {
  ...
  interactionMouseY(event) {
    ...
    if (selected) {
      let selectedWithMouseYInteraction = selected.filter((x) =>
        x.existsInteraction("mouseY")
      );
      if (selectedWithMouseYInteraction.length == 1) {
        ...
        const yDistance =
          this.clickedEvent.clientY - top - (event.clientY - top);
        //console.log(y)
        //RULE OF 3
        let animation_time =
          -1 * ((yDistance * 10) / (top - this.clickedEvent.clientY));
  ...
  initializeAnimationSystem() {
    ...
    for (let i = 0; i < this.animationSystem.length; i++) {
      let depentFrom = this.getMeshByName(
        this.animationSystem[i].main_animation.name
      );
      ...
      for (
        let j = 0;
        j < this.animationSystem[i].dependant_animations.length;
        j++
      ) {
        let dependant = this.getMeshByName(
          this.animationSystem[i].dependant_animations[j].name
        );
        depentFrom.addDependant(dependant);
        dependant.setIsDepentFrom(depentFrom);
        dependant.animationType = "son";
        dependant.timeRelations =
          this.animationSystem[i].dependant_animations[j].time_relations;
      }
    }
  }
}
